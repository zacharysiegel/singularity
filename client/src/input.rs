...
fn click(mouse_position: RenderCoord) {
    ...
    map::click_map(mouse_position);
}

fn hover(mouse_position: RenderCoord) {
    for window in WINDOW_LAYERS {
        let mut window: RwLockWriteGuard<dyn Window> = window.write().unwrap();
        match window.handle_hover(mouse_position) {
            HoverResult::Pass => {}
            HoverResult::Consume => return,
        }
    }
    map::hover_map(mouse_position);
}
