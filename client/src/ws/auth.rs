...
async fn login(lobby_url: &str, password: &str) -> Result<String, AppError> {
    ...
    if !response.status().is_success() {
        return Err(AppError::new(&format!("login failed with status {}", response.status())));
    }
...
async fn create_account(lobby_url: &str, password: &str) -> Result<(), AppError> {
    ...
    if !response.status().is_success() {
        return Err(AppError::new(&format!("create account failed with status {}", response.status())));
    }
