...
async fn get_account_statistics(
    ...
    let statistic_entities: Vec<StatisticEntity> = unwrap_or_500!(statistic_db::get_statistics_by_account(pool.get_ref(), account_id).await);
...
async fn get_game_statistics(
    ...
    let statistic_entities: Vec<StatisticEntity> = unwrap_or_500!(statistic_db::get_statistics_by_game(pool.get_ref(), game_id).await);
    let statistic_serials: Vec<StatisticSerial> = unwrap_or_500!(statistic_entities
        .into_iter()
        .map(|entity| Statistic::try_from(entity).map(|statistic| StatisticSerial::from(&statistic)))
        .collect::<Result<Vec<_>, _>>());
