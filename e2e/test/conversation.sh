...
# Create conversation
CONV=$(curl -s -X POST "$BASE_URL/conversation" -H "Authorization: Bearer $TOKEN_A" -H 'Content-Type: application/json' -d "{\"member_account_ids\":[\"$ACCOUNT_B_ID\"],\"name\":\"Test Chat\"}")
...
STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE_URL/conversation/$CONV_ID/messages" -H "Authorization: Bearer $TOKEN_C" -H 'Content-Type: application/json' -d "{\"conversation_id\":\"$CONV_ID\",\"content\":\"should fail\"}")
...
# Add member C
STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE_URL/conversation/$CONV_ID/member" -H "Authorization: Bearer $TOKEN_A" -H 'Content-Type: application/json' -d "{\"account_id\":\"$(echo "$ACCOUNT_C" | jq -r '.id')\"}")
